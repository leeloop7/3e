<section class="py-12 overflow-hidden" id="presentation">
    <div class="container px-4 mx-auto">
        <div class="flex flex-wrap lg:flex-nowrap items-center">
            <div class="w-full lg:w-1/2">
                <div class="lg:py-6 lg:pr-32 wow animate__animated animate__fadeIn" data-wow-delay=".3s">
                    <div class="mb-4">
                        <span
                            class="text-xs py-1 px-3 text-blue-500 font-semibold bg-blue-50 rounded-xl wow animate__animated animate__fadeInDown"
                            data-wow-delay=".9s">{{__("Kdo smo?")}}</span>
                        <h2 class="text-4xl mt-5 font-bold font-heading wow animate__animated animate__fadeInUp"
                            data-wow-delay=".3s">{{__("Predstavitev")}}</h2>
                    </div>
                    <div class="flex items-start py-4 wow animate__animated animate__fadeInUp" data-wow-delay=".5s">
                        <div>
                            <p class="text-blueGray-400 leading-loose">
                                {{__("3e.presentation.paragraph_1")}}
                            </p>
                        </div>
                    </div>
                    <div class="flex items-start py-4 wow animate__animated animate__fadeInUp" data-wow-delay=".7s">

                        <div>
                            <p class="text-blueGray-400 leading-loose">
                                {{__("3e.presentation.paragraph_2")}}
                            </p>
                        </div>
                    </div>
                    <div class="flex items-start py-4 wow animate__animated animate__fadeInUp" data-wow-delay=".9s">
                        <div>
                            <p class="text-blueGray-400 leading-loose">
                               {!! __("3e.presentation.paragraph_3") !!}
                            </p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="relative w-full lg:w-1/2 my-12 lg:my-0">
                <div class="wow animate__animated animate__fadeInRight" data-wow-delay=".5s">
                    <img class="jump relative mx-auto rounded-xl w-full z-10" src="/imgs/presentation.png" alt=""/>
                    <img class="absolute top-0 left-0 w-40 -ml-12 -mt-12" src="/imgs/elements/blob-tear.svg" alt=""/>
                    <img class="absolute bottom-0 right-0 w-40 -mr-12 -mb-16" src="/imgs/elements/blob-tear.svg"
                         alt=""/>
                </div>
            </div>
        </div>
    </div>
</section>
